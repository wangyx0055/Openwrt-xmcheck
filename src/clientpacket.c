...
int get_raw_packet(struct dhcpMessage *payload, int fd)
{
	...
	packet.ip.check = 0;
	if (check != /*checksum(&(packet.ip), sizeof(packet.ip))*/wrapsum (checksum ((unsigned char *)&packet.ip, sizeof packet.ip, 0))) {
	...
	if (check && check != /*checksum(&packet, bytes)*/wrapsum (checksum ((unsigned char *)&packet.udp, sizeof (packet.udp),
				   checksum ((unsigned char *)&packet.data, ntohs(packet.udp.len) - sizeof(packet.udp), 
